...
UINT8 CUniversialSerialCommunicate::skpxxcx(){
	...
	ret = manageFunc.GetTaxpayerInfo(*g_YwXmlArg, *(g_globalArgLib->m_corpInfo), qtxx, strErr);
	if (SUCCESS != ret)
	{
		m_serialProtocol->Rsp_ERR(strErr);
		return FAILURE;
...
//	memcpy(&request, m_serialProtocol->m_revCmd->revData, sizeof(Common_Request));
	...
	g_YwXmlArg->m_jqbh = (INT8 *)(request.jqbh);
	ret = manageFunc.GetJKSJ(*g_YwXmlArg, *(g_globalArgLib->m_invKind), strErr);
	if (SUCCESS != ret)
	{
		m_serialProtocol->Rsp_ERR(strErr);
		return FAILURE;
...
//		memcpy(request.items[i].xm, m_serialProtocol->m_revCmd->revData+offset, XMMC_LEN);
		...
		pInvDet->m_spmc = (INT8 *)(item.xm);
		FilterSpace(pInvDet->m_spmc);
...
//		memcpy(request.items[i].dj, m_serialProtocol->m_revCmd->revData+offset, DJ_LEN);
		...
		pInvDet->m_dj = atof((INT8 *)item.dj);
		...
		pInvDet->m_spsl = atof((INT8 *)item.sl);	
		...
		pInvDet->m_je = double2int(atof((INT8 *)item.je)*SUM_EXTENSION);	
		...
		pInvDet->m_sl = (float)atof((INT8 *)item.zsl);	
		...
		pInvDet->m_spse = double2int(atof((INT8 *)item.se)*SUM_EXTENSION);	
		...
		pInvDet->m_spdj = atof((INT8 *)item.hsdj);
		...
		pInvDet->m_spje = double2int(atof((INT8 *)item.hsje)*SUM_EXTENSION);	
