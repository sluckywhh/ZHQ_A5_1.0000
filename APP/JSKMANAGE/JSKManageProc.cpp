...
INT32 CJSKManageProc::SKPXXCX_Proc(CUserInfo *pUserInfo, string &strCurTime, string &strErr)
{
	...
	//Case 0x01：获取纳税户信息
	retcode = CJSKInfoFunc::GetCorpInfo(pUserInfo, tax, strErr);
	if(retcode != JSK_SUCCESS)
	{
		return retcode;
	}
...
// 	CUserInfo tmpUserInfo;
// 	tmpUserInfo.Requery();
// 	if(tmpUserInfo.LoadOneRecord() != SQLITE_OK)
// 	{
// 		pUserInfo->AddNew();
// 	}
	pUserInfo->Delete();
...
INT32 CJSKManageProc::BSPXXCX_Proc(CUserInfo *pUserInfo, string &strErr)
{
	INT32 retcode = 0;
...
/*
INT32 CJSKManageProc::JKSJCX_Proc(CInvKind *pInvKind, string &strErr)
{
	DBG_PRINT(("===== JKSJCX_Proc Begin() ======"));
	INT32 retcode = 0;
	INT8 tmpbuf[64];
	UINT8 timeBuf[MAX_DATETIME_LEN+1];
	UINT32 invNum = INVKIND_MAX_NUM;
	UINT8 fplx;
	int i=0;
	CInvKind tmpInvKind[INVKIND_MAX_NUM];

	retcode = CJSKInfoFunc::GetOffLimitInfo(tmpInvKind, invNum, strErr);
	if(retcode != JSK_SUCCESS)
	{
		memset(tmpbuf, 0, sizeof(tmpbuf));
	...
	for(i=0; i<invNum; i++)
	{
		DBG_PRINT(("fplx:  %u  ----  %u", tmpInvKind[i].m_fplx, fplx));
		if(tmpInvKind[i].m_fplx == fplx)
		{
			pInvKind->m_fplx = tmpInvKind[i].m_fplx;
			CJSKInvManageProc::fplx2fplxdm(pInvKind->m_fplx, pInvKind->m_fplxdm);
			pInvKind->m_maxSign = tmpInvKind[i].m_maxSign;
			pInvKind->m_Lxkjsj = tmpInvKind[i].m_Lxkjsj;
			pInvKind->m_Lxssr = tmpInvKind[i].m_Lxssr;
			pInvKind->m_maxSum = tmpInvKind[i].m_maxSum;
			break;
		}
	}
	if(i>=invNum)
	{
		DBG_PRINT(("here ================="));
		pInvKind->m_fplx = tmpInvKind[0].m_fplx;
		CJSKInvManageProc::fplx2fplxdm(pInvKind->m_fplx, pInvKind->m_fplxdm);
		pInvKind->m_maxSign = tmpInvKind[0].m_maxSign;
		pInvKind->m_Lxkjsj = tmpInvKind[0].m_Lxkjsj;
		pInvKind->m_Lxssr = tmpInvKind[0].m_Lxssr;
		pInvKind->m_maxSum = tmpInvKind[0].m_maxSum;
	}
...
INT32 CJSKManageProc::SQSLCX_Proc(CTax *pTax, UINT8 &taxNum, string &strErr)
{
	...
	//Case 0x01：获取纳税户信息
	retcode = CJSKInfoFunc::GetCorpInfo(&userInfo, pTax, strErr);
	if(retcode != JSK_SUCCESS)
	{
		return retcode;
	}

	taxNum = userInfo.m_Slgs;
...
// 		g_gNetArg->SetHashNo((INT8 *)uSksbxx, (INT8 *)uAddrNo);
	...
	case SKSBQTYXXCX_XXLX_WLLQFPJGQR:
		hashFlag = HASH_NO_FLAG_24;
		retcode = CJSKInfoFunc::App2NetInvVol(fplx, outLen, uSksbxx,&invVol, strErr);
		break;
...
INT32 CJSKManageProc::NetPara_Proc(CNetPara *pNetPara, string &strErr)
{
	INT32 retcode = 0;

	pNetPara->Delete();
	if(pNetPara->AddNew() != SQLITE_OK)
	{
